...
export function Globe({ globeConfig, data, onGlobeClick }: WorldProps) {
  const globeRef = useRef<ThreeGlobe | null>(null);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const groupRef = useRef<any>(null);
  const [isInitialized, setIsInitialized] = useState(false);

  // Function to convert 3D world position to lat/lng
  const worldToLatLng = (x: number, y: number, z: number) => {
    ...
    const lng = Math.atan2(normalizedZ, normalizedX) * (180 / Math.PI);
  ...
  const handleClick = (event: any) => {
    ...
    if (point) {
      const { lat, lng } = worldToLatLng(point.x, point.y, point.z);
